...
class Quadrotor(BaseTask):
    ...
    def create_sim(self):
        ...
        self.progress_buf = torch.zeros(
            self.cfg.env.num_envs, device=self.sim_device, dtype=torch.float32) #long
    ...
    def step(self, actions):
        ...
        return self.obs_buf, self.privileged_obs_buf, self.rew_buf, self.reset_buf, self.extras, self.dones
    ...
    def pre_physics_step(self, _actions):
        ...
        self.forces[:] = 0.0
        self.torques[:, :] = 0.0
        # print(f'pre_physics_step: {self.action_input}, upperlimit: {self.action_upper_limits}')
        ...
        self.torques[:, 0] = output_torques_inertia_normalized # Huyen is modifying here!
        self.forces = torch.where(self.forces < 0, torch.zeros_like(self.forces), self.forces)
        # print(f'output_thrusts_mass_normalized: ', output_thrusts_mass_normalized)
        # apply actions
    ...
    def compute_observations(self):
        self.obs_buf[..., :3] = self.root_positions #interpolate(self.root_positions) # TODO: check
    ...
    def compute_reward(self):
        self.rew_buf[:],  self.dones[:], self.reset_buf[:]= compute_quadcopter_reward(
            self.root_positions,
            self.root_quats,
            self.root_linvels,
            self.root_angvels,
...
def compute_quadcopter_reward(root_positions, root_quats, root_linvels, root_angvels, reset_buf, progress_buf, max_episode_length):
    ...
    target_dist = torch.sqrt(root_positions[..., 0] * root_positions[..., 0] +
                             ...
                             (6-root_positions[..., 2]) * (6-root_positions[..., 2]))
    pos_reward = 2.0 / (1.0 + target_dist * target_dist)

    dist_reward = (20.0 - target_dist) / 40.0
    # dist_reward = -torch.exp(-1.0 / (0.1*target_dist*target_dist)) 
    # uprightness
